...
export async function runEngine(opts, tool, args, timeoutMs, signal) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            if (!done) {
                ...
                child.kill('SIGKILL');
                reject(new Error('[dsh-code-graph] 引擎调用超时(' + timeoutMs + 'ms): ' + tool));
            }
        }, timeoutMs);
        const onAbort = () => { if (!done) {
            done = true;
            clearTimeout(timer);
            child.kill('SIGTERM');
            reject(new Error('[dsh-code-graph] 引擎调用被取消: ' + tool));
        } };
...
function invalidateListCache() { lpCache = null; }
export async function listProjects(opts, force = false) {
    if (!force && lpCache && Date.now() - lpCache.at < LP_TTL_MS)
        return lpCache.projects;
...
export async function indexSingleFlight(opts, repoPath, mode, timeoutMs, name) {
...
export async function deleteProject(opts, project) {
    try {
...
function loadState(stateFile) {
    ...
    try {
        if (existsSync(stateFile)) {
            ...
            const sigs = parsed?.sigs && typeof parsed.sigs === 'object' ? parsed.sigs : parsed;
            for (const [k, v] of Object.entries(sigs ?? {})) {
                if (v && typeof v === 'object' && typeof v.sig === 'string')
                    sigStore.set(k, v);
            }
...
export async function recordSig(opts, project, root, stateFile) {
    try {
...
export async function ensureFresh(opts, project, timeoutMs) {
    if (!opts.fresh)
        return { checked: false, drift: false, reindexed: false, note: 'fresh=off' };
    const st = await runEngine(opts, 'index_status', { project }, 30_000);
    const root = st.data?.root_path;
    if (!root || !existsSync(root))
        return { checked: false, drift: false, reindexed: false, note: 'no root_path' };
    let live;
    try {
        live = await liveSig(root);
    }
    catch (e) {
        return { checked: false, drift: false, reindexed: false, note: 'git unavailable: ' + String(e.message).slice(0, 120) };
    }
...
async function evictMulti(opts, canon, keep) {
...
async function pruneSingle(opts, canon, keep) {
    const matching = (await listProjects(opts, true)).filter((p) => canonPath(p.root_path ?? '') === canon);
    const removed = [];
...
export async function branchInventory(opts, canon) {
    loadState(opts.stateFile);
    const reg = regStore[canon];
    if (!reg || (!reg.alias && !reg.single))
        return undefined;
 ...
 */
export async function resolveProjectBranchAware(opts, a, o) {
    // 1) 显式 project：分支层不介入
    if (a.project) {
        const lp = await listProjects(opts);
    ...
    if (!matching.length) {
        ...
        if (!branch) {
            ...
            if (name)
                await recordSig(opts, String(name), canon, opts.stateFile);
        ...
        loadState(opts.stateFile);
        const reg = regStore[canon] ?? (regStore[canon] = {});
    ...
    if (entry?.project && byName.has(entry.project)) {
        if (await deleteProject(opts, entry.project))
...
export async function ripgrepFallback(root, query, signal) {
    const tokens = query.split(/\s+/).filter((t) => t.length >= 2).slice(0, 4);
    if (!tokens.length)
        return { ran: false, reason: 'no usable tokens', hits: [] };
    const es = signal;
    const toHits = (stdout) => stdout.split('\n').filter(Boolean).slice(0, 24);
    // rg 优先
    try {
        ...
        const err1 = e1;
        if (err1.code === 1)
            return { ran: true, engine: 'ripgrep', hits: [] };
        // rg 缺失/其他错误 → grep 兜底
        try {
