...
function engineOptsWith(deps, freshOverride) {
    return freshOverride === undefined ? deps.opts : { ...deps.opts, fresh: freshOverride };
}
/** 项目解析（分支感知）+ 查询前保鲜（仅当前检出分支；显式非当前分支为冻结索引不保鲜）。 */
async function readyProject(deps, a, withFresh) {
    const r = await resolveProjectBranchAware(deps.opts, a);
...
function rowsToObjects(data) {
    ...
    return rows.map((r) => {
        const o = {};
        cols.forEach((c, i) => { o[String(c)] = r[i]; });
        return o;
    });
}
export async function dispatchOp(a, deps) {
    ...
    switch (a.op) {
        ...
        case 'status': {
            ...
            const out = { op: a.op, project: r.project, status: st.data, ...resMeta(r) };
        ...
        case 'index': {
            if (!a.repo_path)
                throw new Error('[dsh-code-graph] op=index 需要 repo_path（canonical 绝对路径）');
        ...
        case 'query': {
            if (!a.query)
                throw new Error('[dsh-code-graph] op=query 需要 query（关键词/符号名；BM25 结构加权排序）');
        ...
        case 'callees': {
            if (!a.name)
                throw new Error('[dsh-code-graph] op=' + a.op + ' 需要 name（函数/方法名）');
            const r = await readyProject(deps, a, true);
        ...
        case 'impact': {
            if (!a.name)
                throw new Error('[dsh-code-graph] op=impact 需要 name（从该符号向上做调用方 BFS）');
            const r = await readyProject(deps, a, true);
            const depth = Math.min(Math.max(a.depth ?? 3, 1), 8);
            const limit = a.max_rows ?? opts.maxRows;
            const frontier = new Set([a.name]);
            ...
            for (let d = 1; d <= depth && frontier.size > 0; d++) {
                ...
                for (const node of [...frontier]) {
                    ...
                    for (const row of rowsToObjects(q.data)) {
                        if (row?.name && !seen.has(String(row.name))) {
                ...
                for (const row of rows)
                    frontier.add(String(row.name));
            }
            return { op: a.op, project: r.project, name: a.name, depth, affected: layers, total: seen.size - 1, ...resMeta(r) };
        }
        case 'trace': {
            if (!a.name)
                throw new Error('[dsh-code-graph] op=trace 需要 name');
            const r = await readyProject(deps, a, true);
            const args = { 'function_name': a.name, project: r.project };
            if (a.mode)
                args.mode = a.mode;
            if (a.direction)
        ...
        case 'snippet': {
            if (!a.name)
                throw new Error('[dsh-code-graph] op=snippet 需要 name（qualified_name 或短函数名，来自 query 结果）');
            const r = await readyProject(deps, a, false);
