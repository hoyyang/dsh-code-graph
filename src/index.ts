...
import type { OpArgs } from './ops.js'
...
interface HostCtx {
  tools: { register(tool: unknown): unknown; get?(name: string, scope?: unknown): unknown }
...
const projectCache = { names: [] as string[] }

async function refreshProjects(opts: EngineOptions): Promise<string[]> {
  try {
    const r = await runEngine(opts, 'list_projects', {}, 20_000)
    const list: any[] = r.data?.projects ?? []
 ...
 */
function stillInstalled(dshHome: string): boolean {
  try {
    for (const dir of readdirSync(join(dshHome, 'profiles'))) {
      try {
        const pkg = JSON.parse(readFileSync(join(dshHome, 'profiles', dir, 'package.json'), 'utf8')) as {
          dsh?: { profile?: { bundles?: string[] } }
          dependencies?: Record<string, string>
        }
        if ((pkg.dsh?.profile?.bundles ?? []).some((b) => String(b).includes('dsh-code-graph'))) return true
        if (pkg.dependencies !== undefined && Object.keys(pkg.dependencies).some((k) => k.includes('dsh-code-graph'))) return true
  ...
  try {
    const reg = JSON.parse(readFileSync(join(dshHome, 'super-injector', 'registry.json'), 'utf8')) as { name?: string }[]
    if (Array.isArray(reg) && reg.some((row) => typeof row?.name === 'string' && row.name.includes('dsh-code-graph'))) return true
...
export function apply(ctx: HostCtx, config: Config): void {
  const stateFile = (): string =>
    join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'dsh-code-graph', 'state.json')
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    execute: async (rawArgs: unknown, toolCtx?: { signal?: AbortSignal }) => {
      const args = rawArgs as OpArgs
    ...
    ctx.effect(() => ctx.commands!.register({
      ...
      handler: async (invocation: { rawInput?: string }) => {
        ...
        if (!raw) {
          ...
          if (!names.length) return { kind: 'success' as const, text: '代码图：暂无已索引项目。用 /codegraph <repo绝对路径> 建索引。' }
          const lines: string[] = ['代码图已索引项目：']
          for (const n of names) {
            try {
              const st = await runEngine(engineOpts(), 'index_status', { project: n }, 30_000)
              const d: any = st.data ?? {}
              const br: string | undefined = d.branch ?? d.git?.branch
              lines.push('- ' + n + '：' + (d.status ?? '?') + '，' + (d.nodes ?? '?') + ' 节点 / ' + (d.edges ?? '?') + ' 边' + (br ? '，branch=' + br : '') + '，root=' + (d.root_path ?? '?'))
            } catch (e) {
              lines.push('- ' + n + '：状态查询失败（' + String((e as Error).message).slice(0, 120) + '）')
            }
          }
        ...
        if (!existsSync(repo)) return { kind: 'error' as const, text: '路径不存在: ' + repo }
        const r = await dispatchOp({ op: 'index', repo_path: repo, mode }, { opts: engineOpts() })
        return { kind: 'success' as const, text: '索引完成：' + JSON.stringify(r) }
      },
  ...
  if (ctx.systemPrompt) {
    ctx.effect(() => ctx.systemPrompt!.section({
      ...
      order: 50,
      text: () => {
        if (!config.hint || !projectCache.names.length) return ''
        return [
  ...
  const toolNameOf = (e: any): string => String(e?.tool ?? e?.name ?? e?.payload?.name ?? e?.data?.name ?? '')

  ctx.effect(() => (ctx as unknown as {
    on(event: string, handler: (payload: any, next: () => Promise<any>) => Promise<any>, opts?: { prepend?: boolean }): unknown
  }).on('agent/pre-step', async ({ agent }: { agent?: any }, next: () => Promise<any>) => {
    const decision = await next()
    try {
      const session = agent?.session
      if (session === undefined || !projectCache.names.length) return decision
  ...
  if (config.gate) {
    const seenCg = new WeakSet<object>()
    const gateLog = (o: Record<string, unknown>): void => {
      try {
        const file = join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'codegraph-gate.log')
        const line = JSON.stringify(o) + '\n'
        appendFileSync(file, line)
        try { if (statSync(file).size > 262144) writeFileSync(file, line) } catch { /* 轮转失败忽略 */ }
      } catch { /* 审计失败不影响主流程 */ }
    }
    ...
    const argsOf = (e: any): Record<string, any> => (e && (e.args ?? e.input ?? e.arguments ?? e.params)) || {}
    const isCodeTargeted = (a: Record<string, any>): boolean => {
      const p = typeof a?.path === 'string' ? a.path : ''
      const inc = typeof a?.include === 'string' ? a.include : ''
      if (inc) {
        if (CODE_IN_GLOB.test(inc)) return true
        if (DOC_IN_GLOB.test(inc)) return false
      }
      if (!p) return true // 缺省 path = 会话工作区（通常是代码仓库）：从严
      if (CODE_EXT.test(p)) return true
      try {
        if (statSync(p).isDirectory()) return REPO_MARKERS.some((m) => existsSync(join(p, m)))
      } catch { /* 路径不存在/glob 形态：放行 */ }
      return false
    }
    ctx.effect(() => (ctx as unknown as {
      on(event: string, handler: (exec: any, next: () => Promise<any>) => Promise<any>): unknown
    }).on('tools/pre-execute', async (exec: any, next: () => Promise<any>) => {
      try {
        const t = String(exec?.name ?? exec?.tool ?? '')
        if (t === 'codegraph') {
          if (exec?.agent) seenCg.add(exec.agent)
          gateLog({ t: Date.now(), ev: 'codegraph' })
          return next()
        }
        if (t !== 'grep') return next()
        const a = argsOf(exec)
        if (!isCodeTargeted(a)) { gateLog({ t: Date.now(), ev: 'grep-allow-noncode', a }); return next() }
        if (!exec?.agent) { gateLog({ t: Date.now(), ev: 'grep-allow-noagent' }); return next() }
        if (seenCg.has(exec.agent)) { gateLog({ t: Date.now(), ev: 'grep-allow-after-codegraph', a }); return next() }
        if (!ctx.tools.get?.('codegraph', exec.agent)) { gateLog({ t: Date.now(), ev: 'grep-allow-no-codegraph-tool', a }); return next() }
        gateLog({ t: Date.now(), ev: 'deny', a })
        return { kind: 'deny' as const, reason: GATE_REJECT }
      } catch (e) {
  ...
  if (config.skill) {
    ctx.effect(() => {
      ...
      try {
        ...
        ctx.logger?.warn?.('[dsh-code-graph] skill 落盘失败: ' + String((e as Error).message))
      }
      return () => {
        ...
        const timer = setTimeout(() => {
          try {
            if (stillInstalled(home)) return
            if (!existsSync(file)) return
            if (readFileSync(file, 'utf8') !== SKILL_MARKDOWN) return
            rmSync(file)
          } catch { /* 清理失败不阻断卸载 */ }
        }, 1500)
  ...
  ctx.effect(() => {
    ...
    return () => {
      try { rmSync(file, { force: true }) } catch { /* 清理失败不阻断卸载 */ }
    }
